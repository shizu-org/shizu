...
#include "Shizu/Runtime/Objects/BigInteger10/fromString.h"
#include "Shizu/Runtime/Objects/BigInteger10/multiply.h"

static void
Shizu_BigInteger10_finalize
  (
    Shizu_State2* state,
...
static void
Shizu_BigInteger10_initializeDispatch
  (
    Shizu_State1* state,
    Shizu_BigInteger10_Dispatch* self
  );

static void
Shizu_BigInteger10_constructImpl
  (
    Shizu_State2* state,
...
Shizu_BigInteger10_getHashValueImpl
  (
    Shizu_State2* state,
    Shizu_BigInteger10* self
  );
...
Shizu_BigInteger10_isEqualToImpl
  (
    Shizu_State2* state,
    Shizu_BigInteger10* self,
...
static Shizu_ObjectTypeDescriptor const Shizu_BigInteger10_Type = {
  .postCreateType = (Shizu_PostCreateTypeCallback*)NULL,
  .preDestroyType = (Shizu_PreDestroyTypeCallback*)NULL,
  .visitType = (Shizu_VisitTypeCallback*)NULL,
  .size = sizeof(Shizu_BigInteger10),
  .construct = &Shizu_BigInteger10_constructImpl,
  .visit = (Shizu_OnVisitCallback*)NULL,
  .finalize = (Shizu_OnFinalizeCallback*)&Shizu_BigInteger10_finalize,
  .dispatchSize = sizeof(Shizu_BigInteger10_Dispatch),
  .dispatchInitialize = (Shizu_OnDispatchInitializeCallback*)&Shizu_BigInteger10_initializeDispatch,
  .dispatchUninitialize = NULL,
};

Shizu_defineObjectType("Shizu.BigInteger10", Shizu_BigInteger10, Shizu_Object);
...
{
  Shizu_Cxx_Debug_assert(NULL != self->p);
...
{
  ((Shizu_Object_Dispatch*)self)->getHashValue = (Shizu_Integer32(*)(Shizu_State2*, Shizu_Object*)) & Shizu_BigInteger10_getHashValueImpl;
  ((Shizu_Object_Dispatch*)self)->isEqualTo = (Shizu_Boolean(*)(Shizu_State2*, Shizu_Object*, Shizu_Value const*)) & Shizu_BigInteger10_isEqualToImpl;
}
  ...
  )
{
  if (self->sign == other->sign) {
    return _Shizu_BigInteger10_compareMagnitudes(state, self, other); 
  } else {
  ...
  )
{
  ...
  return Shizu_BigInteger10_compare(state, self, Shizu_BigInteger10_createFromInteger32(state, other));
  ...
  )
{
  ...
  // Slow path: The signs are equal.
  return Shizu_BigInteger10_compare(state, self, Shizu_BigInteger10_createFromInteger64(state, other));
}
...
static void
Shizu_BigInteger10_constructFromBigInteger10
  (
    Shizu_State2* state,
    Shizu_BigInteger10* SELF,
    Shizu_BigInteger10* other
...
{
  Shizu_Type* TYPE = Shizu_BigInteger10_getType(state);
  Shizu_BigInteger10* SELF = (Shizu_BigInteger10*)Shizu_Value_getObject(&argumentValues[0]);
  ...
  if (numberOfArgumentValues == 1) {
    ...
    if (Shizu_Value_isObject(&argumentValues[1])) {
      ...
      if (Shizu_Types_isSubTypeOf(Shizu_State2_getState1(state), Shizu_State2_getTypes(state), object->type, Shizu_BigInteger10_getType(state))) {
        Shizu_BigInteger10_constructFromBigInteger10(state, SELF, (Shizu_BigInteger10*)object);
        ...
        _Shizu_BigInteger10_fromString(state, (Shizu_String*)object, &SELF->sign, &SELF->p, &SELF->n);
      } else {
        Shizu_State2_setStatus(state, Shizu_Status_ArgumentTypeInvalid);
        Shizu_State2_jump(state);
      }
    } else if (Shizu_Value_isInteger32(&argumentValues[1])) {
      _Shizu_BigInteger10_fromInteger32(state, Shizu_Runtime_Extensions_getInteger32Value(state, &argumentValues[1]), &SELF->sign, &SELF->p, &SELF->n);
    #if 1 == Shizu_Configuration_WithInteger64
    } else if (Shizu_Value_isInteger64(&argumentValues[1])) {
      _Shizu_BigInteger10_fromInteger64(state, Shizu_Runtime_Extensions_getInteger64Value(state, &argumentValues[1]), &SELF->sign, &SELF->p, &SELF->n);
    #endif
    } else {
      Shizu_State2_setStatus(state, Shizu_Status_ArgumentTypeInvalid);
      Shizu_State2_jump(state);
  ...
  )
{
  ...
  if (Shizu_Types_isSubTypeOf(Shizu_State2_getState1(state), Shizu_State2_getTypes(state), Shizu_Object_getObjectType(state, otherObject), Shizu_BigInteger10_getType(state))) {
    Shizu_BigInteger10* x = self;
    Shizu_BigInteger10* y = (Shizu_BigInteger10*)otherObject;
    return x->sign == y->sign && !_Shizu_BigInteger10_compareMagnitudes(state, x, y);
  } else {
    return Shizu_Boolean_False;
  }
}
  ...
  )
{
  Shizu_Type* TYPE = Shizu_BigInteger10_getType(state);
  Shizu_ObjectTypeDescriptor const* DESCRIPTOR = Shizu_Type_getObjectTypeDescriptor(Shizu_State2_getState1(state), Shizu_State2_getTypes(state), TYPE);
  Shizu_BigInteger10* SELF = (Shizu_BigInteger10*)Shizu_Gc_allocateObject(state, DESCRIPTOR->size);
...
{
  // If we could store somewhere Shizu_Integer32_Minimum and Shizu_Integer32_Maximum as Shizu_BigInteger10 values,
  // then a test if this would overflow would be very fast.
  if (Shizu_BigInteger10_compareInteger32(state, self, Shizu_Integer32_Minimum) < 0) {
    Shizu_State2_setStatus(state, Shizu_Status_ConversionFailed);
    Shizu_State2_jump(state);
  }
  if (Shizu_BigInteger10_compareInteger32(state, self, Shizu_Integer32_Maximum) > 0) {
    Shizu_State2_setStatus(state, Shizu_Status_ConversionFailed);
    Shizu_State2_jump(state);
  }
  Shizu_Integer32 v = 0;
...
{
  // If we could store somewhere Shizu_Integer75_Minimum and Shizu_Integer64_Maximum as Shizu_BigInteger10 values,
  // then a test if this would overflow would be very fast.
  if (Shizu_BigInteger10_compareInteger64(state, self, Shizu_Integer64_Minimum) < 0) {
    Shizu_State2_setStatus(state, Shizu_Status_ConversionFailed);
    Shizu_State2_jump(state);
  }
  if (Shizu_BigInteger10_compareInteger64(state, self, Shizu_Integer64_Maximum) > 0) {
    Shizu_State2_setStatus(state, Shizu_Status_ConversionFailed);
    Shizu_State2_jump(state);
  }
  Shizu_Integer64 v = 0;
  ...
  )
{
  ...
  Shizu_BigInteger10* RESULT = (Shizu_BigInteger10*)Shizu_Gc_allocateObject(state, DESCRIPTOR->size);
  Shizu_Object_construct(state, (Shizu_Object*)RESULT);
  _Shizu_BigInteger10_multiply(state, self, other, &RESULT->sign, &RESULT->p, &RESULT->n);
  ((Shizu_Object*)RESULT)->type = TYPE;
  return RESULT;
}
  ...
  )
{
  ...
  // x = 0 or n = 0 => x * 10^n = x
  if (Shizu_BigInteger10_isZero(state, self) || 0 == n) {
    return self;
  }
  ...
  )
{
  ...
  if (self->p[0] == 0 || self->n <= n) {
    return Shizu_BigInteger10_zero(state);
  }
  Shizu_Type* TYPE = Shizu_BigInteger10_getType(state);
  Shizu_ObjectTypeDescriptor const* DESCRIPTOR = Shizu_Type_getObjectTypeDescriptor(Shizu_State2_getState1(state), Shizu_State2_getTypes(state), TYPE);
  Shizu_BigInteger10* RESULT = (Shizu_BigInteger10*)Shizu_Gc_allocateObject(state, DESCRIPTOR->size);
