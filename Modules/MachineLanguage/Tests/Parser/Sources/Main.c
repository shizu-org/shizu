...
static void parse(Shizu_State2* state, Shizu_String* relativePath) {
  ...
  Shizu_Environment* environment = Shizu_Environment_getEnvironment(state, Shizu_State2_getGlobalEnvironment(state), Shizu_String_create(state, "MachineLanguage", strlen("MachineLanguage")));
  ...
  )
{
  ...
  if (!setjmp(jumpTarget.environment)) {
    ...
    //
    //parse(state, Shizu_String_concatenate(state, path, Shizu_String_create(state, "HelloWorld1.ml", strlen("HelloWorld1.ml"))));
    //
    parse(state, Shizu_String_concatenate(state, path, Shizu_String_create(state, "MyProcedure1.ml", strlen("MyProcedure1.ml"))));
