...
static void scan(Shizu_State2* state, Shizu_String* relativePath) {
  ...
  Shizu_Environment* dataDefinitionLanguageEnvironment = Shizu_Environment_getEnvironment(state, Shizu_State2_getGlobalEnvironment(state), Shizu_String_create(state, "DataDefinitionLanguage", strlen("DataDefinitionLanguage")));
  Shizu_CxxProcedure* p = Shizu_Environment_getCxxProcedure(state, dataDefinitionLanguageEnvironment, Shizu_String_create(state, "createScanner", strlen("createScanner")));
  ...
  {
    Shizu_Environment* tokenTypeEnvironment = Shizu_Environment_getEnvironment(state, Shizu_Environment_getEnvironment(state, Shizu_State2_getGlobalEnvironment(state), Shizu_String_create(state, "DataDefinitionLanguage", sizeof("DataDefinitionLanguage")-1)), Shizu_String_create(state, "TokenType", sizeof("TokenType") - 1));
  ...
  )
{
  ...
  if (!setjmp(jumpTarget.environment)) {
    ...
    //
    scan(state, Shizu_String_concatenate(state, path, Shizu_String_create(state, "integers.ddl", strlen("integers.ddl"))));
    //
    scan(state, Shizu_String_concatenate(state, path, Shizu_String_create(state, "reals.ddl", strlen("reals.ddl"))));
    //
    scan(state, Shizu_String_concatenate(state, path, Shizu_String_create(state, "strings.ddl", strlen("strings.ddl"))));
    //
    scan(state, Shizu_String_concatenate(state, path, Shizu_String_create(state, "empty-list.ddl", strlen("empty-list.ddl"))));
    //
    scan(state, Shizu_String_concatenate(state, path, Shizu_String_create(state, "empty-map.ddl", strlen("empty-map.ddl"))));
